...
namespace VRSStudio.Input.CustomGrab
{
    ...
    public class CustomGrabbable : MonoBehaviour
    {
        ...
        private static readonly List<CustomGrabbable> customGrabbables = new List<CustomGrabbable>();

        public static CustomGrabbable GetNearestGrabbable(Vector3 position, out float distance)
        {
            float nearestDistance = 1000f;
            CustomGrabbable nearest = null;
            foreach (var gbb in customGrabbables)
            {
                var dist = (gbb.transform.position - position).sqrMagnitude;
            ...
            distance = Mathf.Sqrt(nearestDistance);
            return nearest;
        }

        private void OnEnable()
        {
            customGrabbables.Add(this);
        }

        private void OnDisable()
        {
            customGrabbables.Remove(this);
        }
