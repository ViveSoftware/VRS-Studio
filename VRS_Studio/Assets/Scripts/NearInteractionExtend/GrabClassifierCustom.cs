...
{
	public class GrabClassifierCustom
	{
		...
		void DEBUG(string msg)
		{
			if (Log.EnableDebugLog)
				Log.d(LOG_TAG, (m_Hand.IsLeftHand ? "Left" : "Right") + " " + m_Hand.Joint + ", " + msg, true);
		}
		...
		#region Properties from HandInteractor
		private HandInteractorCustom m_Hand = null;
		...
		public GrabClassifierCustom(
			HandInteractorCustom actor,
		...
		private void HandleGrabClassification(
			...
			HandInteractableCustom actable,
		...
		private bool FixedUpdateGrabStateChange(HandInteractableCustom actable, GrabMode mode)
		{
			...
			if (m_GrabClassifiedEx && !m_GrabClassified
				&& mode == GrabMode.ReleaseGrab && m_Hand.GrabbedObject == actable)
			{
				stateChanged = true;
				// Updates grab status only when state changes.
				m_GrabClassifiedEx = m_GrabClassified;
		...
		public bool FixedUpdateGrabBegin(out HandInteractableCustom grabbedObject)
		{
			grabbedObject = null;

			// Already grabbing or lost tracking.
			if (m_Hand.isGrabbing || !m_Hand.isTracked) { return false; }

			foreach (var actable in m_Hand.GrabCandidates)
			{
		...
		public bool FixedUpdateGrabEnd(out HandInteractableCustom releasedObject)
		{
			releasedObject = null;

			// Nothing to release.
			if (!m_Hand.isGrabbing) { return false; }

			if (FixedUpdateGrabStateChange(m_Hand.GrabbedObject, GrabMode.ReleaseGrab))
			{
				releasedObject = m_Hand.GrabbedObject;
