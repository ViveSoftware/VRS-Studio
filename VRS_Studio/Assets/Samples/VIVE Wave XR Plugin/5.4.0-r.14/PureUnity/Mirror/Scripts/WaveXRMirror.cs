...
public class WaveXRMirror : MonoBehaviour
{
	...
	[Tooltip("Custom culling matrix can avoid some object disappear only in one eye.")]
	public bool useCustomCullingMatrix = false;
	public float customCullingfov = 120;
	public float customCullingNear = 0.01f;
	public float customCullingFar = 1000;
	public float customCullingAspect = 1;

    [Tooltip("Not to do occlusion culling can avoid artifact")]
    public bool useOcclusionCulling = false;

	[Tooltip("If debug, show indicators and the camera's object")]
	public bool debugMode = false;

	[Tooltip("For debug.  Indicate the position of the left eye reflection camera.  Can be empty.")]
    public Transform reflectCameraIndicatorL;
    [Tooltip("For debug.  Indicate the position of the right eye reflection camera.  Can be empty.")]
    public Transform reflectCameraIndicatorR;
    [Tooltip("For debug.  Indicate the position of the center eye reflection camera.  Can be empty.")]
    public Transform reflectCameraIndicatorC;

	private Matrix4x4 worldToCameraMatrixL, worldToCameraMatrixR, worldToCameraMatrixC;
	...
	private bool GetWorldCameraMatrixFromDisplayProvider()
	{
		...
		// Make the center camera's matrix to be the average of left and right.
		// Assume rotation is the same.
		if (passCount == 0)
		{
			worldToCameraMatrixC = Camera.main.worldToCameraMatrix;
		}
        else
        {
			worldToCameraMatrixC = worldToCameraMatrixL;
			worldToCameraMatrixC.m03 = (worldToCameraMatrixL.m03 + worldToCameraMatrixR.m03) / 2;
			worldToCameraMatrixC.m13 = (worldToCameraMatrixL.m13 + worldToCameraMatrixR.m13) / 2;
			worldToCameraMatrixC.m23 = (worldToCameraMatrixL.m23 + worldToCameraMatrixR.m23) / 2;
        }
...
#pragma warning restore
    ...
    private void RenderCamera(Camera cam, Renderer rend, Camera.StereoscopicEye eye, ref RenderTexture reflectionTexture)
	{
		...
		*/
		...
		Matrix4x4 cullingPoint;
		Matrix4x4 projectionMatrix;
		if (cam.stereoEnabled)
		{
			if (debugMode && eye == Camera.StereoscopicEye.Left && reflectCameraIndicatorL != null)
			{
				reflectCameraIndicatorL.position = newEyePos;
			}
			else if (debugMode && eye == Camera.StereoscopicEye.Right && reflectCameraIndicatorR != null)
			{
				reflectCameraIndicatorR.position = newEyePos;
			}

			projectionMatrix = cam.GetStereoProjectionMatrix(eye);

			// Both eye use the same culling point to avoid some object disappear in only one eye.
			cullingPoint = worldToCameraMatrixC * reflection;
			var posC = worldToCameraMatrixC.inverse.MultiplyPoint(Vector3.zero);
			if (debugMode && reflectCameraIndicatorC != null)
			{
				reflectCameraIndicatorC.position = reflection.MultiplyPoint(posC);
			}

			// Keep the camera's position and rotation as center eye can avoid skybox render from differnt pose.
			reflectionCamera.transform.position = reflection.MultiplyPoint(posC);
            reflectionCamera.transform.rotation = GetRotation(worldToCameraMatrixC);
        }
        else
		{
			projectionMatrix = cam.projectionMatrix;
			cullingPoint = projectionMatrix;
		}
		...
		Matrix4x4 cullingProjMatrix;
		if (useCustomCullingMatrix)
		{
			cullingProjMatrix = Matrix4x4.Perspective(customCullingfov, customCullingAspect, customCullingNear, customCullingFar);
		}
		else
		{
			cullingProjMatrix = projectionMatrix;
		}

		reflectionCamera.projectionMatrix = projectionMatrix;
		reflectionCamera.cullingMatrix = cullingProjMatrix * cullingPoint;
	...
	private void CopyCameraProperties(Camera src, Camera dest)
	{
		...
		if (src.clearFlags == CameraClearFlags.Skybox)
		{
			...
			if (mysky != null)
			{
                Material skyboxMaterial = sky != null ? sky.material : RenderSettings.skybox;
                if (!skyboxMaterial)
                {
                    mysky.enabled = false;
                }
	...
	private void CreateMirrorObjects(Camera currentCamera, Camera.StereoscopicEye eye, out Camera reflectionCamera, ref RenderTexture reflectionTexture)
	{
		...
		if (!reflectionTexture || m_OldReflectionTextureSize != m_TextureSize)
		{
			...
			reflectionTexture.antiAliasing = m_Antialiasing;
		...
		{
			GameObject go = new GameObject("MirrorReflectCamera" + GetInstanceID() + "for " + currentCamera.GetInstanceID(), typeof(Camera), typeof(Skybox));
