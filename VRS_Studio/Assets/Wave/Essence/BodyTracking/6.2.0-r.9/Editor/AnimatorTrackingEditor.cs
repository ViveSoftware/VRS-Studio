...
{
	[CustomEditor(typeof(AnimatorTracking))]
	public class AnimatorTrackingEditor : UnityEditor.Editor
	{
		SerializedProperty m_Tracking, m_AvatarHeight, m_BoneHip, m_AvatarOffset, m_AvatarScale, m_JointCoordinate;
		...
		public override void OnInspectorGUI()
		{
			...
			AnimatorTracking myScript = target as AnimatorTracking;
			...
			{
				EditorGUILayout.HelpBox(
					"Sets up the avatar's height instead of using the Animator avatar's (head's height - toes's height).",
					MessageType.Info);
			...
			if (GUI.changed)
				EditorUtility.SetDirty((AnimatorTracking)target);
		}
	}
}
#endif
