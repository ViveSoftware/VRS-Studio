...
namespace Wave.Essence.Hand.Interaction
{
	...
	public static class DataWrapper
	{
...
#if UNITY_EDITOR
			bool enableDirectPreview = EditorPrefs.GetBool("Wave/DirectPreview/EnableDirectPreview");
			if (!enableDirectPreview) { return false; }
#endif
			return HandManager.Instance.IsHandPoseValid(isLeft);
		}

		/// <summary>
		/// Get the position of a specific joint.
		/// </summary>
		/// <param name="id">The type of joint to get.</param>
		/// <param name="position">The reference to store the position of the joint.</param>
		/// <param name="isLeft">True if the hand is left; otherwise, false.</param>
		/// <returns>True if the joint position is successfully obtained; otherwise, false.</returns>
		public static bool GetJointPosition(JointType id, ref Vector3 position, bool isLeft)
		{
			if (!Validate()) { return false; }
			return HandManager.Instance.GetJointPosition((HandManager.HandJoint)id, ref position, isLeft);
		}
		...
		/// <param name="isLeft">True if the hand is left; otherwise, false.</param>
		/// <returns>True if the joint rotation is successfully obtained; otherwise, false.</returns>
		public static bool GetJointRotation(JointType id, ref Quaternion rotation, bool isLeft)
		{
			if (!Validate()) { return false; }
			return HandManager.Instance.GetJointRotation((HandManager.HandJoint)id, ref rotation, isLeft);
		}

		public static bool useXRDevice => HandManager.Instance.UseXRDevice;
	}
	...
	public class HandData
	{
		...
		public bool useXRDevice => DataWrapper.useXRDevice;
	...
	public static class CachedHand
	{
		...
		private static void GetJointData(JointType id, ref JointData data, bool isLeft)
		{
			...
			DataWrapper.GetJointPosition(id, ref data.position, isLeft);
			DataWrapper.GetJointRotation(id, ref data.rotation, isLeft);
	...
	public class HandGrabState : GrabState
	{
		...
		private class FingerPinchState
		{
			...
			const string LOG_TAG = "Wave.Essence.Hand.Interaction.HandGrabState.FingerPinchState ";
	...
	{
		[SerializeField]
		private bool m_EnableConstraint;
		public bool IsEnable() => m_EnableConstraint;
		...
		{
			m_EnableConstraint = in_EnableConstraint;
			m_Value = in_Value;
		}
