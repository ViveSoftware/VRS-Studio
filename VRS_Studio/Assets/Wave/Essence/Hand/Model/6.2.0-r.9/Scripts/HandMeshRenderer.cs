...
namespace Wave.Essence.Hand.Model
{
	...
	public class HandMeshRenderer : MonoBehaviour
	{
		...
		{
			if (!InputDeviceHand.IsTracked(IsLeft)) return false;
		...
		void OnEnable()
		{
			Application.onBeforeRender += this.UpdateWristPose;
		...
		#region BeforeRenderEvent
		private static int updateFrame = -1;
		private static Dictionary<bool, Pose> wristPoses = new Dictionary<bool, Pose>()
		{
			{true, Pose.identity},
			{false, Pose.identity},
		};
		private static void UpdateXRWristPose(InputDevice inputDevice, bool IsLeft)
		{
			if (inputDevice.TryGetFeatureValue(CommonUsages.handData, out UnityEngine.XR.Hand hand) &&
				hand.TryGetRootBone(out Bone palmBone) && palmBone.TryGetParentBone(out Bone wristBone) &&
				wristBone.TryGetPosition(out Vector3 position) && wristBone.TryGetRotation(out Quaternion rotation))
			{
				wristPoses[IsLeft] = new Pose(position, rotation);
			}
		}
		private static void UpdateHandData()
		{
			...
			{
				HandManager.Instance.UpdtaeHandData();
			}
			else
			{
				...
				{
					if (InputDeviceHand.IsHandDevice(inputDevices[i], true))
					{
						UpdateXRWristPose(inputDevices[i], true);
					}
					else if (InputDeviceHand.IsHandDevice(inputDevices[i], false))
					{
						UpdateXRWristPose(inputDevices[i], false);
					}
				}
			}
		}
		private void SetWristLocalPose(Vector3 position, Quaternion rotation)
		{
			if (BonePoses[(int)HandManager.HandJoint.Wrist] != null)
			{
				BonePoses[(int)HandManager.HandJoint.Wrist].localPosition = position;
				BonePoses[(int)HandManager.HandJoint.Wrist].localRotation = rotation;
			}
		}
		private void UpdateWristPose()
		{
			UpdateHandData();
			if (HandManager.Instance != null && !HandManager.Instance.UseXRDevice)
			{
				...
				if (HandManager.Instance.GetJointPosition(HandManager.HandJoint.Wrist, ref position, IsLeft) &&
					HandManager.Instance.GetJointRotation(HandManager.HandJoint.Wrist, ref rotation, IsLeft))
				{
					SetWristLocalPose(position, rotation);
				}
			}
			else
			{
				SetWristLocalPose(wristPoses[IsLeft].position, wristPoses[IsLeft].rotation);
			}
		}
		#endregion
	}
