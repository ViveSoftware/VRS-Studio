...
namespace Wave.Essence.Controller.Model
{
	public class RenderModel : MonoBehaviour
	{
		...
		class Component
		{
			...
			{
				this.isVisible = isVisible;
				renderer.enabled = showState && isVisible;
			}
		...
		private void onLoadController(WVR_DeviceType type)
		{
			...
			if (Interop.WVR_GetWaveRuntimeVersion() < 2 && !EnableDirectPreview)
			{
				...
				{
					if (controllerSpawned != null) { Destroy(controllerSpawned); }
		...
		IEnumerator SpawnRenderModel()
		{
			...
			if (childArray == null || childArray.Length != modelResource.sectionCount)
			{
				...
				{
					deleteChild("SpawnRenderModel() Creates a new child array.");
					childArray = new Component[modelResource.sectionCount];
		...
		private void deleteChild(string reason)
		{
			...
			PrintInfoLog("deleteChild() " + reason);
			...
			if (childArray != null)
			{
				...
				for (int i = 0; i < ca; i++)
				{
					if (childArray[i] != null)
					{
						MeshRenderer renderer = childArray[i].GetRenderer();
						if (renderer != null &&
							renderer.material != null)
						{
							if (renderer.material.mainTexture != null)
							{
								PrintInfoLog("deleteChild texture: " + renderer.material.mainTexture.name);
								Destroy(renderer.material.mainTexture);
								renderer.material.mainTexture = null;
							}
							PrintInfoLog("deleteChild material: " + renderer.material.name);
							Destroy(renderer.material);
							renderer.material = null;
						}

						GameObject CM = childArray[i].GetObject();
						if (CM != null)
						{
		...
		private void DeleteControllerWhenDisconnect()
		{
			...
			if (this.connected)
			{
				...
				if (tmprenderModelName != renderModelName)
				{
					deleteChild("Destroy controller prefeb because " + type + " render model is different");
				}
			}
			else
			{
